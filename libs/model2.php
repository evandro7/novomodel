...
abstract class model2 {
	...
	public function save( $obj )
	{
		...
		echo 'Filha de '.get_parent_class($obj) .'<br>';
		$campos ='';
		$valores ='';
			
		$pai = get_parent_class($obj);
		
		
		if ($pai != ''){
			$refpai = new ReflectionClass( $pai );
			$campopai[]=$refpai->getProperties();
			foreach ($campopai[0] as $k){
				$propai = $refpai->getProperty($k->name);
				//echo substr($prop->getName(), 0, 3).'<br>';
				if ($propai->isPrivate() ) {
					$propai->setAccessible(true);
					//echo "Campo: " . $prop->getName().' - valor: '.$prop->getValue($usuario) . "<br>";
					echo 'K '.$k->name.'<br>';
					$campos .=$propai->getName().', ';
					$valores .="'".$propai->getValue($obj)."', ";
				}
			}
		}
		
		//$reflection = new ReflectionAnnotatedClass( $class );  
		//$this->tabela = $reflection->getAnnotation('Table')->value;   
		
		
		$campo[]=$ref->getProperties();
		
		
		
		foreach ($campo[0] as $k){
	...
	function delete(object $obj){
		...
		try{
			$queryAtual = $this->bd->prepare($sqlDel);
			$queryAtual->execute($parametros);
			$this->RegAf=$queryAtual->rowCount();
		} catch (PDOException $e) {
				echo htmlentities('Houve algum erro com a atualização: '. $e->getMessage());
